...
这个例子提供了一个小的应用程序，其中使用操纵杆按钮来触发使用中断的USART通信，并使用USART固件库。
...
在两个板（A或B板）中，当使用USARTx时，在stm32f0xxit文件中使用usartxirqhandler来管理数据传输。
...
SysTick被配置为每10毫秒产生中断。在SysTick ISR中有一个专用的计数器用来切换LD1，每100毫秒显示固件正在运行
...
一个定义的通信超时是为了确保如果USART通信被破坏，应用程序将不会被卡住
...
您可以通过main中定义的usertimeout来调整这个超时。h文件取决于CPU的频率和应用程序条件
（中断例程、传输的数据数量、包体、CPU频率…）。
