...
这个例子提供了如何通过超终端使用USART的自动包的功能的描述。
...
首先，配置了USART1（STM32F051R8T6设备）或USART2（STM32F072VBT6设备）
用StartBit测量方法自动检测波特拉特（任何一个从1点开始的字符都应该发送给USART来检测波特率）。
...
在配置之后，USART会等待通过超终端接收到数据，从而启动波特率阶段的自动检测。这个阶段的结束由ABRF标志监视。
...
自动检测的结果是由EVAL板led显示的。如果在自动包的相位ABRE标志中发生了错误，那么就打开了LED3。如果这个阶段成功完成，那么LED2就会被打开，接收到的数据会被传输回超终端。

You can repeat this procedure by changing the BaudRate on HyperTerminal side.

您可以通过改变超终端的波特率来重复这个过程。
